function multsel (scrollerObj) {
    ...
    parent.on('click', '.js-view', function (e) {
        ...
        var $t_parent = $(this).parents('.multse-select-wrap')
        var scrollerData = $t_parent.find('.select-scroller').attr('data-scroller')
        var idStr = 'scroller' + scrollerData

        if ($t_parent.hasClass('expand')) {
            $t_parent.removeClass('expand').find('.select-scroller').stop('true','true').slideUp('linear')
                .end().find('.arrow').removeClass('rotate')
        } else {
            parent.removeClass('expand').find('.select-scroller').stop('true','true').slideUp('linear')
                .end().find('.arrow').removeClass('rotate')

            $t_parent.addClass('expand').find('.select-scroller').stop('true','true').slideDown('linear', function () {
    ...
    // 多选操作
    parent.on('click', '.select-item', function () {
        var self = $(this)
...
function multselCheck(domParent) {
    var checkedHtml = '',
        ...
        optionText = '',
        optionItem = '',
        placeholder = domParent.parents('.multse-select-wrap').attr('data-placeholder'),
    ...
    if(!checkedHtml) {
        checkedHtml = '<span class="placeholder">' + placeholder + '</span>'
        hiddenText = ''
    }
