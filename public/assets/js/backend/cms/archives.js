define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template'], function ($, undefined, Backend, Table, Form, Template) {
    ...
    var Controller = {
        index: function () {
            ...
            var columns = [
                ...
                },
                {
                    field: 'title', title: __('Title'), align: 'left', formatter: function (value, row, index) {
                        return '<div class="tdtitle"'+ (row['is_sensitive'] == 1 ? ' style="color: #db3733"' : '') +'>' + value + '</div>' + Table.api.formatter.flag.call(this, row['flag'], row, index);
                    }
                },
                {field: 'recommends', title: __('Recommends'),operate: false},
                {field: 'user.nickname', title: __('User_id'),operate: false},
                {field: 'image', title: __('Image'), operate: false, formatter: Table.api.formatter.image},
                {field: 'views', title: __('Views'), operate: 'BETWEEN', sortable: true,operate: false},
                //{field: 'comments', title: __('Comments'), operate: 'BETWEEN', sortable: true,operate: false},
                ...
                {field: 'status', title: __('Status'), searchList: {"normal": __('Status normal'), "hidden": __('Status hidden'), "rejected": __('Status rejected'), "pulloff": __('Status pulloff'), "unreleased": __('Status unreleased')}, formatter: Table.api.formatter.status},
                {
                    field: 'operate',
                    title: __('Operate'),
                    table: table,
            ...
            // 重发
            $(document).on("click", "#btn-repeat-file", function () {
                var ids = Table.api.selectedids(table);

                Fast.api.ajax({
                    url: 'cms/archives/repeat',
                    data: {ids: ids}
                }, function (data) {
            ...
            require(['jstree'], function () {
                ...
                $('#channeltree').on("changed.jstree", function (e, data) {
                    ...
                    // if (data.selected.join(",") == 69) {
                    //     window.location.href = '/admin/cms/archives?addtabs=1&recommend=69';
                    // } else {
                    //     table.bootstrapTable('refresh', {});
                    // }
                    table.bootstrapTable('refresh', {});
        ...
        add: function () {
            $(document).on('change', '#c-channel_id', function () {
                Fast.api.ajax({
                    ...
                    $('#image-size').text("推荐尺寸("+data.text+")");
                    $("#extend").html(data.html);

                    //会员项必填
                    if (data.mustUser == 1) {
                        $('input[name="row[user_id]"]').attr('data-rule', 'required');
                    }
                ...
                $("#c-tags").data("params", function(){
                    return {"custom[type]": localStorage.getItem('last_channel_id')};
                });
            });
