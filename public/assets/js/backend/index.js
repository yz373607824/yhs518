define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            ...
            //读取Uwebadmin的更新信息
            $.ajax({
                url: Config.uwebadmin.api_url + '/news/index',
                type: 'post',
                dataType: 'jsonp',
            ...
            if (localStorage.getItem("fastep") == "installed") {
                $.ajax({
                    url: Config.uwebadmin.api_url + '/addon/recommend',
                    type: 'post',
                    dataType: 'jsonp',
                    success: function (ret) {
                        require(['template'], function (Template) {
                            var install = function (name, title) {
                                Fast.api.ajax({
                                    url: 'addon/install',
                                    data: {name: name, faversion: Config.uwebadmin.version}
                                }, function (data, ret) {
            ...
            var task = function () {
                $.ajax({
                    ...
                    type: 'get',
                    success: function (ret) {
                        Toastr.success(__('Task success'));
                    }, error: function (e) {
            ...
            var checkupdate = function (ignoreversion, tips) {
                $.ajax({
                    url: Config.uwebadmin.api_url + '/version/check',
                    type: 'post',
                    data: {version: Config.uwebadmin.version},
                    dataType: 'jsonp',
                    success: function (ret) {
            ...
            if (Config.uwebadmin.checkupdate && ignoreversion !== "*") {
            ...
            //手动触发定时任务
            $("a[data-toggle='task']").on('click', function () {
                task();
            });
            ...
            var multiplenav = Config.uwebadmin.multiplenav;
            ...
            if ($("ul.sidebar-menu li.active a").size() > 0) {
                ...
                if (Config.uwebadmin.multiplenav) {
