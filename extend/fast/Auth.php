...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = Db::name($this->config['auth_group_access'])
                ...
                ->field('aga.uid,aga.group_id,ag.id,ag.pid,ag.name,ag.rules,ag.channel_rules')
    ...
    public function getChannelRuleIds($uid)
    {
        //读取用户所属用户组
        $groups = $this->getGroups($uid);
        $ids = []; //保存用户所属用户组设置的所有栏目权限规则id
        foreach ($groups as $g) {
            $ids = array_merge($ids, explode(',', trim($g['channel_rules'], ',')));
        }
        $ids = array_unique($ids);
        return $ids;
    }
