...
class Expert extends Frontend
{
    ...
    public function index()
    {
        ...
        if (!empty($adept)) {
            $list->where('FIND_IN_SET(:adept, a.relation_adept)', ['adept'=>$adept]);
        }
        if (!empty($province)) {
        ...
        if (!empty($keywords)) {
            //增加精通行业/领域、专家擅长、问题、答案、描述、评论搜索条件
            $expertOnline = db('expert_online')->whereLike('title', '%' . $keywords . '%', 'OR')
                ->whereLike('question_description', '%' . $keywords . '%', 'OR')
                ->whereLike('replycontent', '%' . $keywords . '%', 'OR')
                ->column('expert_id');
            $expertLocale = db('expert_locale')->whereLike('title', '%' . $keywords . '%', 'OR')
                ->whereLike('question_description', '%' . $keywords . '%', 'OR')
                ->column('expert_id');
            $order = db('order')->where('type','in',[1,2])->whereLike('appraise|replyappraise', '%' . $keywords . '%')->column('shop_id');
            $expertId = implode(',',array_unique(array_merge($expertOnline, $order, $expertLocale)));
            if ($expertId) {
                $list->where('a.nickname LIKE "%'.$keywords.'%" OR a.technosphere LIKE "%'.$keywords.'%" OR a.adept LIKE "%'.$keywords.'%" OR a.id IN ('.$expertId .')');
            } else {
                $list->whereLike('a.nickname|a.technosphere|a.adept', '%'.$keywords.'%');
            }
        }
        //前后处理
    ...
    public function detail()
    {
        ...
        //问题列表
        $questionList = OnlineModel::where('expert_id', $expert['id'])
            ->order('createtime', 'desc')
            ->paginate(config('page_size.appraise'),false,[
                'type' => '\\app\\common\\controller\\ExpertPage'
            ]);
     ...
     * 专家在线问题详情页
     */
    public function expertQuestionDetail() {
        $id = $this->request->param('id');
        if (empty($id)) {
            $this->error(__('Parameter error'));
        }
        //当前在线问题详情
        $questionDetailInfo = Online::alias('a')
            ->where('a.id',$id)
            ->join('user' . ' u','a.user_id = u.id','LEFT')
            ->join('user_expert' . ' e','a.expert_id = e.id','LEFT')
            ->field('a.*,u.nickname,u.avatar,u.level,e.nickname as expertnickname,e.avatar as expertavatar,e.technosphere,e.achievement,e.level as expertlevel')
            ->find();
        //等级的转换
        $questionDetailInfo['leveltext'] = __('Level ' . $questionDetailInfo['expertlevel']);
        $this->assign("questionDetailInfo", $questionDetailInfo);
        $level = 'one';
        switch ($questionDetailInfo['expertlevel']) {
            case '1' :
                $level = 'one';
                break;
            case '2' :
        ...
        $this->assign('localePrice', config('site.locale_' . $level . '_price'));
        //追问追答
        $questionAndAnswer = Online::alias('a')
            ->where('a.id',$id)
            ->join('order' . ' o','a.id = o.goods_id','LEFT')
            ->where('o.is_show', '1')
            ->field('o.*')
            ->find();
        $this->assign("questionAndAnswer", $questionAndAnswer);
        return $this->view->fetch('expert_question_detail');
    }
