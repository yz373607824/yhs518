...
class Auth
{
    ...
    public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        $data = [
            ...
            'level'    => 1,
            'score'    => 0,
            'points'   => config('site.register_gold_sum'),    //金币，平台附送
            'no'       => 'SH' . Random::numeric(10),            //随机ID
            'avatar'   => '',
        ];
        $params = array_merge($data, [
            'nickname'  => date('Ym') . Random::alpha(4),
            'salt'      => Random::alnum(),
            'jointime'  => $time,
            'joinip'    => $ip,
            'logintime' => $time,
...
//        $field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? 'mobile' : 'username');
        $field = Validate::regex($account, '/^1\d{10}$/') ? 'mobile' : 'username';
    ...
    public function logout()
    {
        ...
        //会员下线
        db('user')->where('id', $this->_user->id)->update(['expire' => 0, 'online' => '0']);
    ...
    public function direct($user_id)
    {
        ...
        if ($user)
        {
            ...
            $user->logintime = $time;
            //在线有效期
            $user->expire = $time + $this->keeptime;
            //登录状态
            $user->online = '1';
            //登录次数
            $user->loginnum += 1;
