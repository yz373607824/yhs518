...
class Expert extends Frontend
{
    ...
    protected $noNeedLogin = ['index', 'detail', 'all_online_page', 'online_page', 'locale_page'];
...
//            $county = $this->request->get('county', null);
            $address = $this->request->get('address', null);
            ...
            if (!empty($row['province'])) {
                $list->where('a.province', $row['province']);
            }
            if (!empty($row['city'])) {
                $list->where('a.city', $row['city']);
            }
            if (!empty($row['county'])) {
                $list->where('a.county', $row['county']);
            }
            if (!empty($keywords)) {
            ...
            foreach ($list as $expert) {
                $level = __('Level ' . $expert['level']);
                if ($expert['online'] == 1) {
                    $status = '<div class="state st-green">在线</div>';
                } else {
                    $status = '<div class="state st-green">24小时在线</div>';
                    //$status = '<div class="state">休息中</div>';
                }
                $link = '';
                if (strpos($expert['service'],'online') !== false) {
                    $link .= '<a href="'.url('/mobile/expert/detail', ['id' => $expert['id']]).'" class="btn bg-y">我要提问</a>';
                }
                if (strpos($expert['service'],'locale') !== false) {
                    $link .= '<a href="'.url('/mobile/expert/detail', ['id' => $expert['id']]).'" class="btn bg-g">我要现场服务</a>';
                }
                $url = url('/mobile/expert/detail', ['id' => $expert['id']]);
                $html .= <<<HTML
                <li>
                    <a href="{$url}" class="img-block"><img src="{$expert['avatar']}" alt="{$expert['nickname']}"></a>
                    <div class="info-block">
                        <div class="title">
                            <a href="{$url}" class="tt-box">
                        ...
                        <div class="expert-id">
                            <div class="id-num">编号：{$expert['job_number']}</div>
                            <div class="great">
                                <i class="iconfont icon-cai"></i>
                                <span class="great-num">{$expert['likes']}</span>
                            </div>
                        </div>
                        <div class="info">
                        ...
                        <p class="skill">精通行业/领域：{$expert['technosphere']}</p>
                        <p class="skill">服务企业：{$expert['company']}</p>
                        <div class="btn-block">
                            {$link}
                        </div>
                    </div>
...
HTML;
        ...
        return $this->view->fetch('expert_list');
    }
    ...
    public function detail()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $item) {
                $appraisetime = date('Y-m-d H:i', $item['appraisetime']);
                $star = '';
                for ($i = 0; $i < $item['star']; $i++) {
                    $star .= '<img src="/assets/static_mobile/images/star.jpg" alt="">';
                }
                $replyappraise = '';
                $item['avatar'] = empty($item['avatar']) ? '/assets/img/avatar.png' : $item['avatar'];

                if (!empty($item['replyappraise'])) {
                    $replytime = date('Y-m-d H:i', $item['replytime']);
                    $replyappraise = <<<EOT
                        <div class="answer">
                                <div class="triangle"></div>
                                <div class="tt"><span class="tt-1">专家回复</span><span class="tt-2">{$replytime}</span></div>
                                <p class="info">{$item['replyappraise']}</p>
                        </div>
EOT;
                }
                $html .= <<<HTML
                <li class="QA-item">
                            <div class="left-block"><img src="{$item['avatar']}" alt="{$item['nickname']}"></div>
                            <div class="right-block">
                                <div class="hd">
                                    <div class="hd-left">
                                        <div class="name">{$item['nickname']}</div>
                                        <div class="time">{$appraisetime}</div>
                                    </div>
                                    <div class="hd-right">
...
HTML;
     ...
     * 全网提问页面
     */
    public function all_online_page() {
        if (!$this->auth->id) {
            $this->error(__('请登录后再操作'), url('/mobile/user/login'));
        }

        $this->view->assign('seo_title', '全网提问');
        $this->view->assign('seo_keywords', '全网提问');
        $this->view->assign('seo_description', '全网提问');
        $template = preg_replace('/\.html$/', '', 'all_ask_questions.html');
        return $this->view->fetch('expert/' . $template);
    }

    /**
     * 在线服务页面
     */
    public function online_page() {
        if (!$this->auth->id) {
            $this->error(__('请登录后再操作'), url('/mobile/user/login'));
        }
        ...
        $this->assign('id', $id);
        $level = 'one';
        switch ($this->request->param('level')) {
            case '1' :
                $level = 'one';
                break;
            case '2' :
        ...
        $this->view->assign('seo_title', '在线服务');
        $this->view->assign('seo_keywords', '在线服务');
        $this->view->assign('seo_description', '在线服务');
        $template = preg_replace('/\.html$/', '', 'ask_questions.html');
        return $this->view->fetch('expert/' . $template);
    }
    ...
    public function locale_page() {
        ...
        $this->view->assign('seo_title', '现场服务');
        $this->view->assign('seo_keywords', '现场服务');
        $this->view->assign('seo_description', '现场服务');
        $this->assign('localePrice', config('site.locale_' . $level . '_price'));
        $template = preg_replace('/\.html$/', '', 'live_service.html');
        return $this->view->fetch('expert/' . $template);
    }
    ...
    public function online()
    {
        ...
        $this->result(['url' => url('/mobile/user/pay', ['trade_sn' => $data['trade_sn']]), 'price' => $data['price']], 1);
    }
    ...
    public function locale()
    {
        ...
        if (!$this->auth->verify_id && !$this->auth->expert_id && !$this->auth->enterprise_id) {
            $this->error(__('请先进行个人身份认证或专家认证或企业认证'), url('/mobile/user/idAuthentication'), ['token' => $this->request->token($tokenName)]);
        }
    ...
    private function _asynchronousMessage($trade_sn)
    {
        ...
        if ($order && $order['type'] == '1' && $order['status'] == '1') {
            ...
            foreach ($userList as $item) {
                ...
                $url = url('/mobile/user/grabOnlineService', '', true, true);
                $data = ['type' => '全网提问', 'time' => date('Y-m-d H:i', $order['createtime']), 'title' => $online['title']];
                //发送专家的微信信息
                Frontend::sendTplMsg($item['openid'], '您好，有用户提交了全网提问，你可以抢单回答！', $url, $data);
            }
            if (count($list) > 0) {
        ...
        if ($price == 0) {
            ...
            if (in_array($order['type'], ['1', '2'])) {
                ...
                if ($order['type'] == 1) {
                    ...
                    $user_url = url('/mobile/user/myReservation', '', true, true);
                ...
                if (!empty($order['shop_id'])) {
                    ...
                    $this->_asynchronousMessage($order['trade_sn']);
                }
