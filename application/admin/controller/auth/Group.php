...
class Group extends Backend
{
    ...
    protected $groupdata = [];
    //无需要权限判断的方法
    protected $noNeedRight = ['roletree', 'channeltree'];
    ...
    public function channelauth($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $params['channel_rules'] = explode(',', $params['channel_rules']);
            $parentmodel = model("AuthGroup")->get($params['pid']);
            if (!$parentmodel) {
                $this->error(__('The parent group can not found'));
            }
            // 父级别的规则节点
            $parentrules = explode(',', $parentmodel->channel_rules);
            // 当前组别的规则节点
            $currentrules = $this->auth->getChannelRuleIds();
            $rules = $params['channel_rules'];
            // 如果父组不是超级管理员则需要过滤规则节点,不能超过父组别的权限
            $rules = in_array('*', $parentrules) ? $rules : array_intersect($parentrules, $rules);
            // 如果当前组别不是超级管理员则需要过滤规则节点,不能超当前组别的权限
            $rules = in_array('*', $currentrules) ? $rules : array_intersect($currentrules, $rules);
            $params['channel_rules'] = implode(',', $rules);
            if ($params) {
                Db::startTrans();
                try {
                    $row->save($params);
                    ...
                    foreach ($children_auth_groups as $key => $children_auth_group) {
                        ...
                        $childparams[$key]['channel_rules'] = implode(',', array_intersect(explode(',', $children_auth_group->channel_rules), $rules));
                    }
    ...
    public function channeltree()
    {
        ...
        if (($pid || $parentGroupModel) && (!$id || $currentGroupModel)) {
            $id = $id ? $id : null;
            $ruleList = collection(model('app\admin\model\cms\Channel')->field('*, name title')->order('id ASC weigh DESC')->select())->toArray();
            //读取父类角色所有节点列表
            $parentRuleList = [];
            if (in_array('*', explode(',', $parentGroupModel->channel_rules))) {
                $parentRuleList = $ruleList;
            } else {
                $parentRuleIds = explode(',', $parentGroupModel->channel_rules);
                foreach ($ruleList as $k => $v) {
                    if (in_array($v['id'], $parentRuleIds)) {
                        $parentRuleList[] = $v;
                    }
            ...
            $groupTree = new Tree();
            //当前所有正常规则列表
            $ruleTree->init($parentRuleList, 'parent_id');
            //角色组列表
            $groupTree->init(collection(model('AuthGroup')->where('id', 'in', $this->childrenGroupIds)->select())->toArray());
            //读取当前角色下规则ID集合
            $adminRuleIds = $this->auth->getChannelRuleIds();
            //是否是超级管理员
            $superadmin = $this->auth->isSuperAdmin();
            //当前拥有的规则ID集合
            $currentRuleIds = $id ? explode(',', $currentGroupModel->channel_rules) : [];
            ...
            if (!$id || !in_array($pid, $this->childrenGroupIds) || !in_array($pid, $groupTree->getChildrenIds($id, true))) {
                ...
                foreach ($parentRuleList as $k => $v) {
                    ...
                    if ($v['parent_id'] && !in_array($v['parent_id'], $parentRuleIds)) {
                        continue;
                    }
                    $state = array('selected' => in_array($v['id'], $currentRuleIds) && !in_array($v['id'], $hasChildrens));
                    $nodeList[] = array('id' => $v['id'], 'parent' => $v['parent_id'] ? $v['parent_id'] : '#', 'text'
                    => __($v['title']), 'type' => 'menu', 'state' => $state);
                }
                $this->success('', null, $nodeList);
            } else {
                $this->error(__('Can not change the parent to child'));
