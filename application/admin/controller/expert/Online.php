...
class Online extends Backend
{
    ...
    /**
     * Online模型对象
     * @var \app\admin\model\expert\Online
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\expert\Online;
        $this->view->assign("isReplyList", $this->model->getIsReplyList());
        $this->view->assign("isRollbackList", $this->model->getIsRollbackList());
    }
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
				...
				$row['replycontent'] = mb_substr($row['replycontent'], 0, 10) . (mb_strlen($row['replycontent']) > 10 ? '...' : '');
				$order = Order::get(['type'=>config('order_type.online'), 'goods_id'=>$row['id']]);
            ...
            if ($params) {
                try {
                    ...
                    if ($result !== false) {
                        //更新订单的shop_id
                        if ($flag) {
                            \app\admin\model\Order::update(['shop_id' => $params['expert_id']], ['type' => 1, 'goods_id' => $ids]);
                            $this->createTips($user_tips, 1);
                        }
